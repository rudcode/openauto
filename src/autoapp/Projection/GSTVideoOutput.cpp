...
//                        s.erase(m.position(), m.position()+m.length());
//                    }
                    ...
                    if (!posix_spawn_file_actions_init(&fa)) {
                        if (!posix_spawn_file_actions_adddup2(&fa, p_stdin[0], 0)) {
                            if (!posix_spawn_file_actions_adddup2(&fa, p_stdout[1], 1)) {
                                char * const launch[] = {"sh", "-c", "gst-launch fdsrc fd=0 timeout=1000 do-timestamp=true " \
                            "! queue ! h264parse " \
                            ...
                            "max-lateness=1000000000 sync=false async=false 2>&1", nullptr};
                                if (!(e = posix_spawn(&gstpid, "/bin/sh", &fa, nullptr, launch , environ))) {
                ...
                bool GSTVideoOutput::open() {
                    ...
                    gst_cmd << "gst-launch fdsrc fd=0 ";
                    gst_cmd << "timeout=1000 do-timestamp=true ";
                    gst_cmd << "! queue ! h264parse ";
                    gst_cmd << "! vpudec low-latency=true framedrop=true framedrop-level-mask=0x200 ";
                    gst_cmd << " frame-plus=1 ";
                    gst_cmd << "! mfw_isink name=mysink axis-left=0 axis-top=0 disp-width=800 ";
                    gst_cmd << "disp-height=480 max-lateness=1000000000 sync=false async=false";
                    LOG(DEBUG) << gst_cmd.str();
...
//                    ::write(p_stdin[1], buf.cdata, buf.size+1);
