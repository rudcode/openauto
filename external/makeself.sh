...
test -d "$archdir" || {
  echo "Error: $archdir does not exist."
  rm -f "$tmpfile"
  exit 1
}
if test "$QUIET" = "n"; then
...
if test x"$ENCRYPT" = x"openssl"; then
  ...
  { eval "$ENCRYPT_CMD -in $tmpfile -out ${tmpfile}.enc" && mv -f ${tmpfile}.enc $tmpfile; } ||
    {
      echo Aborting: could not encrypt temporary file: "$tmpfile".
      rm -f "$tmpfile"
      exit 1
    }
fi
...
if test "$SHA256" = y; then
  SHA_PATH=$(
    exec <&- 2>&-
    which shasum || command -v shasum || type shasum
  )
  if test -x "$SHA_PATH"; then
    ...
    SHA_PATH=$(
      exec <&- 2>&-
      which sha256sum || command -v sha256sum || type sha256sum
    )
...
if test "$NOMD5" = y; then
  ...
  MD5_PATH=$(
    exec <&- 2>&-
    which md5sum || command -v md5sum || type md5sum
  )
  test -x "$MD5_PATH" || MD5_PATH=$(
    exec <&- 2>&-
    which md5 || command -v md5 || type md5
  )
  test -x "$MD5_PATH" || MD5_PATH=$(
    exec <&- 2>&-
    which digest || command -v digest || type digest
  )
  PATH=$OLD_PATH
